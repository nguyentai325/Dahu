...
int main(int argc, char** argv)
{
  ...
  std::string inputDirectory = "/home/minh/Documents/code/WATERSHED/resize_jpegs";
  std::string outputDirectory = "/home/minh/Documents/MATLAB/Saliency-Objectness-master/Data2/Dahu";
  //std::cout << inputDirectory << std::endl;
  DIR *directory_folder_input = opendir (inputDirectory.c_str());
  DIR *directory1 = opendir (outputDirectory.c_str());
  struct dirent *_dirent_inputfolder = NULL;
  if(directory_folder_input == NULL)
  {
  ...
  while((_dirent_inputfolder = readdir(directory_folder_input)) != NULL)
  {

    if ((std::string(_dirent_inputfolder->d_name) != ".") and (std::string(_dirent_inputfolder->d_name) != ".."))
        {


            std::string fileinputfolder = inputDirectory + "/" +std::string(_dirent_inputfolder->d_name);

            std::string name_interfolder= std::string(_dirent_inputfolder->d_name);
            std::cout << name_interfolder << std::endl;


            std::stringstream output_background;
            output_background<<outputDirectory<<"/"<<std::string(_dirent_inputfolder->d_name);

            std::string folderCreateCommand1 = "mkdir " + output_background.str();
            system(folderCreateCommand1.c_str());

            DIR *directory_interfolder = opendir (fileinputfolder.c_str());

            struct dirent *_dirent_interfolder = NULL;

            while((_dirent_interfolder = readdir(directory_interfolder)) != NULL)
            {

              if ((std::string(_dirent_interfolder->d_name) != ".") and (std::string(_dirent_interfolder->d_name) != ".."))
              {

                  std::string fileinterfolder = fileinputfolder + "/" +std::string(_dirent_interfolder->d_name);
                  std::string name_folder = std::string(_dirent_interfolder->d_name);
                  std::cout << name_folder<< std::endl;

                  std::stringstream output_scene;
                  output_scene<<output_background.str()<<"/"<<std::string(_dirent_interfolder->d_name);

                  std::string folderCreateCommand2 = "mkdir " + output_scene.str();
                  system(folderCreateCommand2.c_str());


                  DIR *directory_image = opendir (fileinterfolder.c_str());

                  struct dirent *_dirent_image = NULL;

                  while((_dirent_image = readdir(directory_image)) != NULL)
                  {

                    if ((std::string(_dirent_image->d_name) != ".") and (std::string(_dirent_image->d_name) != ".."))
                    {
                        std::string fileName = fileinterfolder + "/" +std::string(_dirent_image->d_name);
                      ...
                      tree_t T;
                      {
                        ...
                        image2d <uint8_t>  distance_map_gray(D);
                        extension::fill(distance_map_gray, 0);

                        image2d <uint8_t>  distance_map_gray1(D);
                        extension::fill(distance_map_gray1, 0);
                        ...
                        mln_foreach(auto x, T.nodes())
                        {
                            mln_foreach (auto p, x.proper_pset())
                            {
                                ...
                                distance_map_gray(F.point_at_index(p)) = distance_map(F.point_at_index(p))[0]/3+ 	distance_map(F.point_at_index(p))[1]/3 + distance_map(F.point_at_index(p))[2]/3		;
                            }
                        }
                        ...
                        mln_foreach(auto x, T.nodes())
                        {
                                mln_foreach (auto p1, x.proper_pset())
                                {
                                        ...
                                        if (distance_map_gray(F.point_at_index(p1)) > max_gray)
                                        {
                                                max_gray = distance_map_gray(F.point_at_index(p1)) ;
                                        }
                                }
                        }
                        ...
                        mln_foreach(auto x, T.nodes())
                        {
                                mln_foreach (auto p1, x.proper_pset())
                                {
                                        distance_map_gray1(F.point_at_index(p1)) =  (255 * float(distance_map_gray(F.point_at_index(p1)))/ float(max_gray) );
                                }
                        }


                        //io::imsave(distance_map, "dmap1.png");
                        fileName = output_scene.str() + "/" + std::string(_dirent_image->d_name);

                        io::imsave(distance_map_gray1, fileName.c_str());

                    }
                  }
                  closedir(directory_image);
              }
            }
            closedir(directory_interfolder);
        }
  }
  closedir(directory_folder_input);
