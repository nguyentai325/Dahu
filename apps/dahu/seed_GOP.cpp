...
int main(int argc, char** argv)
{
  ...
  tree_t T;
  {
    ...
    std::vector<unsigned>  enqueue(nodenumbers);
    std::fill (enqueue.begin(),enqueue.end(),0);
    enqueue[0]  = 1;


    point2d seedpoints;
    seedpoints = point2d (100,100);

    unsigned start = pmap(seedpoints);
    ...
    std::cout << "dist  "  << unsigned(dist[start][0])  << unsigned(dist[start][1])<< unsigned(dist[start][2])  << std::endl;
    mln_foreach(auto x, T.nodes())
    {
        mln_foreach (auto p, x.proper_pset())
        {

            distance_map(F.point_at_index(p)) = 	  dist[x.id()]  ;
            distance_map1(F.point_at_index(p)) = distance_map(F.point_at_index(p))[0]/3 + distance_map(F.point_at_index(p))[1]/3+ distance_map(F.point_at_index(p))[2]/3;

        }

    }

    io::imsave(distance_map, "dmap.png");
    io::imsave(distance_map1, "dmap_gray.png");
