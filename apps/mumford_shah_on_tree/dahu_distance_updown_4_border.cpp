...
int main(int argc, char** argv)
{
  ...
  std::string inputDirectory = "/home/minh/Documents/code/code_edwin/build/apps/mumford_shah_on_tree/test";
  std::string outputDirectory = "/home/minh/Documents/code/code_edwin/build/apps/mumford_shah_on_tree/output";
	...
	if ((std::string(_dirent->d_name) != ".") and (std::string(_dirent->d_name) != "..") )
		{
		  ...
		  std::string name_image = std::string(_dirent->d_name); 
		  std::string fileName = inputDirectory + "/" +std::string(_dirent->d_name); 
		  
		  std::string delimiter = ".";
		  std::string token = name_image.substr(0, name_image.find(delimiter)); // token is "scott"
		  ...
		  tree_t T;
		  {
			...
			for (int k = 0 ; k < 4 ; ++k)
			{
				...
				{	
					...
					io::imsave(distancemap_final_gray, fileName_top.c_str());	
				}
				...
				{	
					...
					io::imsave(distancemap_final_gray, fileName_bottom.c_str());	
				}
				...
				{	
					...
					io::imsave(distancemap_final_gray, fileName_left.c_str());	
				}		
				...
				{	
					...
					io::imsave(distancemap_final_gray, fileName_right.c_str());	
				}	
				
						
				std::string fileName_combined = outputDirectory + "/" + "combined_" + std::string(_dirent->d_name);				
				io::imsave(distancemap_final_gray, fileName_combined.c_str());	
			...
			std::string nameofimage = token + ".png";
		
			fileName = outputDirectory + "/" + nameofimage;
