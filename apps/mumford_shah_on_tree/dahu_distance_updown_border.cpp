...
int main(int argc, char** argv)
{
  ...
  std::string inputDirectory = "/home/minh/Documents/code/code_edwin/program/apps/mumford_shah_on_tree/test_resized";
  std::string outputDirectory = "/media/minh/DATA/Study/database/smartdoc2015/out_data";
	...
	if ((std::string(_dirent->d_name) != ".") and (std::string(_dirent->d_name) != "..") )
		{
		  ...
		  tree_t T;
		  {
			...
			std::clock_t time_tree_done = std::clock();
			std::cout << float( time_tree_done - begin_time ) /  CLOCKS_PER_SEC <<std::endl;
			...
			{
				if (is_border(p,height,width) == true)
				{
			...
			std::clock_t time_dahu = std::clock();
			std::cout << float( time_dahu - time_tree_done ) /  CLOCKS_PER_SEC << std::endl;
