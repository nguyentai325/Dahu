...
namespace mln
{
	...
	image2d<uint8_t> abc(I& ima, T& tree )
	{
		unsigned nodenumbers = tree.nodes().size();   

		auto& nodes1 = tree._get_data()->m_nodes;
		auto& S     = tree._get_data()->m_S;
		auto& pmap  = tree._get_data()->m_pmap;		
		
		box2d D = pmap.domain();
		image2d<uint8_t> vkl(D );
...
int main(int argc, char** argv)
{
  ...
  std::string inputDirectory = "/media/minh/DATA/Study/database/smartdoc2015/GOP";
  std::string outputDirectory = "/media/minh/DATA/Study/database/smartdoc2015/GOP_out";
	...
	if ((std::string(_dirent->d_name) != ".") and (std::string(_dirent->d_name) != "..") )
		{
		  ...
		  tree_t T;
		  {
			...
			*/
			...
			image2d<uint8_t>  dcm(D);
			
			dcm = abc(ima, T);
			io::imsave(dcm, fileName.c_str());		
