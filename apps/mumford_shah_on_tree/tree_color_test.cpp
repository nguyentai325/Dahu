...
int main(int argc, char** argv)
{
  ...
  tree_t T;
  {
	...
	std::cout << "T real size "  << T.realsize()<< std::endl;
	...
	unsigned nodenumbers = t[1].nodes().size();   
	...
	mln_foreach(auto x, t[1].nodes())
	{
		...
		std::vector <uint8_t>  under(nodenumbers, 0);
		...
		std::string filename = std::to_string(x.id()) + "_"+  argv[1];
		io::imsave(under_ima, filename);
