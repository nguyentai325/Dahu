...
namespace mln
{
    ...
    image2d<lab<float> > convert_rgb_2_lab( const image2d<rgb8>& input)
    {
...
int main(int argc, char** argv)
{
  ...
  const char* output_path1 = argv[6];
  const char* output_path2= argv[7];
  const char* output_path3 = argv[8];
  ...
  std::string outputDirectory_mtos = "/home/minh/Documents/code/code_edwin/build/apps/papier/Dahu_lab/lab";
  std::string outputDirectory_fusion = "/home/minh/Documents/code/code_edwin/build/apps/papier/Dahu_lab/fusion";
  std::string outputDirectory_L = "/home/minh/Documents/code/code_edwin/build/apps/papier/Dahu_lab/L";
  std::string outputDirectory_a = "/home/minh/Documents/code/code_edwin/build/apps/papier/Dahu_lab/a";
  std::string outputDirectory_b = "/home/minh/Documents/code/code_edwin/build/apps/papier/Dahu_lab/b";
...
//  }
  ...
  while((_dirent = readdir(directory)) != NULL)
  {
    ...
    if ((std::string(_dirent->d_name) != ".") and (std::string(_dirent->d_name) != "..") )
    {
          ...
          //std::string fileName = "/home/minh/Documents/code/code_edwin/build/apps/papier/test_resized/VID_20160921_1611070.png";
            ...
            //image2d<lab<float> > ima_lab = convert_rgb_2_lab(ima);




            image2d<lab<float> > ima_lab1 = convert_rgb_2_lab(ima);
...
//            image2d<uint8> L(D);
            ...
            image2d<lab<float> > f = addborder(ima_lab1, lexicographicalorder_less<lab<float> >());
        ...
        //    // Initiation
            ...
            mln_foreach(auto x, T.nodes())
            {
                mln_foreach (auto p, x.proper_pset())
                {
                    for(int i = 0; i < 3 ; i++)
                    {
                        distance_map(F.point_at_index(p))[i] = max_temp_node[x.id()][i] - min_temp_node[x.id()][i];
                    }
                }
            }

            std::string fileout = output_path;
            io::imsave(distance_map, fileout.c_str());

            image2d<uint8>distance_L =  dahu_distance( t[0], U0 );
            image2d<uint8>distance_a =  dahu_distance( t[1], U1 );
            image2d<uint8>distance_b =  dahu_distance( t[2], U2 );

            image2d<rgb8> distance_fusion(U0.domain() );
            mln_foreach(point2d p1, U0.domain())
            {
                distance_fusion(p1)[0] = distance_L(p1);
                distance_fusion(p1)[1] = distance_a(p1);
                distance_fusion(p1)[2] = distance_b(p1);
            }
            ...
            io::imsave(distance_fusion, fileName.c_str());

            fileName = outputDirectory_L + "/" + std::string(_dirent->d_name);
            io::imsave(distance_L, fileName.c_str());

            fileName = outputDirectory_a + "/" + std::string(_dirent->d_name);
            io::imsave(distance_a, fileName.c_str());

            fileName = outputDirectory_b + "/" + std::string(_dirent->d_name);
            io::imsave(distance_b, fileName.c_str());
