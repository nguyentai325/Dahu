...
int main(int argc, char** argv)
{
  ...
  const char* output_path_max = argv[5];
  const char* output_path_euclidean = argv[6];
  const char* output_path_sum = argv[7];
  ...
  std::string outputDirectory_max = "/home/minh/Documents/code/code_edwin/build/apps/papier/path_cost/max";
  std::string outputDirectory_euclidean = "/home/minh/Documents/code/code_edwin/build/apps/papier/path_cost/euclidean";
  std::string outputDirectory_sum = "/home/minh/Documents/code/code_edwin/build/apps/papier/path_cost/sum";
  ...
  while((_dirent = readdir(directory)) != NULL)
  {
    ...
    if ((std::string(_dirent->d_name) != ".") and (std::string(_dirent->d_name) != "..") )
    {
          ...
          tree_t T;
            ...
            image2d<uint8> dahu_max(dahu_MTOS.domain());
            image2d<float> dahu_euclidean(dahu_MTOS.domain());
            image2d<uint8> dahu_euclidean_nor(dahu_MTOS.domain());

            image2d<uint8> dahu_sum(dahu_MTOS.domain());


            mln_foreach(auto p, dahu_MTOS.domain())
            {
                dahu_max(p) = std::max(std::max(dahu_MTOS(p)[0], dahu_MTOS(p)[1]),dahu_MTOS(p)[2]);
                dahu_euclidean(p) = std::sqrt(dahu_MTOS(p)[0]* dahu_MTOS(p)[0] + dahu_MTOS(p)[1]* dahu_MTOS(p)[1]+ dahu_MTOS(p)[2] * dahu_MTOS(p)[2]);
                dahu_sum(p) = dahu_MTOS(p)[0]/3 + dahu_MTOS(p)[1]/3 + dahu_MTOS(p)[2]/3;
            }
            ...
            mln_foreach(auto p, dahu_MTOS.domain())
            {
                if (dahu_euclidean(p) > max_pixel)
                    max_pixel = dahu_euclidean(p);
            ...
            mln_foreach(auto p, dahu_MTOS.domain())
            {
                dahu_euclidean_nor(p) = dahu_euclidean(p)/ max_pixel *255;
            }
            ...
            mln_foreach(auto p, dahu_MTOS.domain())
            {
                ...
                if (dahu_max(p) <= alpha)
                    dahu_max(p) = 0;
                else if (dahu_max(p) >= beta)
                    dahu_max(p) = 255;
                else
                    dahu_max(p) = uint8(float(dahu_max(p) - alpha)/ float(beta - alpha) *255);

                // dahu_euclidean
                if (dahu_euclidean_nor(p) <= alpha)
                    dahu_euclidean_nor(p) = 0;
                else if (dahu_euclidean_nor(p) >= beta)
                    dahu_euclidean_nor(p) = 255;
                else
                    dahu_euclidean_nor(p) = uint8(float(dahu_euclidean_nor(p) - alpha)/ float(beta - alpha) *255);

                // dahu_sum
                if (dahu_sum(p) <= alpha)
                    dahu_sum(p) = 0;
                else if (dahu_sum(p) >= beta)
                    dahu_sum(p) = 255;
                else
                    dahu_sum(p) = uint8(float(dahu_sum(p) - alpha)/ float(beta - alpha) *255);
            }
            ...
            fileName = outputDirectory_max + "/" + std::string(_dirent->d_name);
            io::imsave(dahu_max, fileName.c_str());

            fileName = outputDirectory_euclidean + "/" + std::string(_dirent->d_name);
            io::imsave(dahu_euclidean_nor, fileName.c_str());

            fileName = outputDirectory_sum + "/" + std::string(_dirent->d_name);
            io::imsave(dahu_sum, fileName.c_str());
