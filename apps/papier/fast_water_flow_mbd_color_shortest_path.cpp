...
//    std::cout << int(dahu_map(p_center)[0])  << "  " << int(dahu_map(p_center)[1]) << "  "  <<int(dahu_map(p_center)[2])  << std::endl;
    ...
    box2d Dom = I.domain();
    image2d<point2d > parent(Dom);
    image2d<V> waterflow_map = waterflow_mbd_distance_2_points( I,  p_topleft,  p_center, waterflow_distance, parent );
    io::imsave(waterflow_map, "dmap2.png");
    std::vector<point2d>  shortestPath = tracePath (parent, p_center);

    image2d<uint8_t> pathimage(Dom);
    ...
    io::imsave(path_waterflow, "path.pgm");
    ...
    image2d<V> Dahuflow_map = fast_dahu_2_points( I,  p_topleft,  p_center, dahuflow_distance, parent_dahu );
    io::imsave(Dahuflow_map, "dmap_dahu.png");

    std::vector<point2d>  shortestPath_dahu = tracePath_dahu(parent_dahu, p_center);

    image2d<uint8_t> pathimage_dahu(D);

    for (int j = 0 ; j < shortestPath_dahu.size() ; j++)
    {
        point2d p = shortestPath_dahu[j];
        //printf("-> (%d,%d) ",p[0],p[1]);
        pathimage_dahu(p) = 255;
    }

    io::imsave(pathimage_dahu, "path_dahu.pgm");
    ...
    std::string fileout = outputDirectory + "/" + "water_" + fileName.substr(fileName.find("/") + 1);;
