...
//  auto area = morpho::paccumulate(T, T._get_data()->m_pmap, acc);
  ...
  //std::cout << "index   "  << F.point_at_index(F.index_of_point(point2d(1,1)))  << std::endl;
    ...
    std::vector<std::vector<point2d> >  pixel_set(nodenumbers);

    mln_foreach(auto x, T.nodes())
    {
        std::vector<uint8>  dejavu(nodenumbers);
        ...
        mln_foreach (auto y, T.nodes())
        {
            if (dejavu[y.get_parent_id()] == 255)
                dejavu[y.id()] = dejavu[y.get_parent_id()];

            mln_foreach (auto p, y.proper_pset())
            {

                node_ima(F.point_at_index(p)) = dejavu[y.id()];
            }
        }

        /////////////////////////////////////////////////////////

        rect2d r = make_rectangle2d(5, 5);
        image2d<uint8> node_ima_dilate = morpho::structural::dilate(node_ima, r);
        image2d<uint8> node_ima_erode = morpho::structural::erode(node_ima, r);
        ...
        mln_foreach (auto p, D)
        {
            ...
            if (node_ima_dilate(p)== 255 and node_ima(p) == 0)
            {
                sum_ext = sum_ext + U(p);
                area_ext = area_ext + 1;
            }

            if (node_ima_erode(p) == 0 and node_ima(p) == 255)
            {
                sum_int = sum_int + U(p);
                area_int = area_int + 1;
            }
        ...
        average_gradient[x.id()] = std::abs(sum_ext/(area_ext+epsilon) - sum_int/(area_int+epsilon));
        if (average_gradient[x.id()] > 20)

            std::cout << x.id()   <<  "  "  << average_gradient[x.id()]  << std::endl;
